...
function excel(Location, i) {
			...
			if (ex[e][j] == '.' || ex[e][j].value == undefined || ex[e][j].value == null || ex[e][j].value == false || ex[e][j] == 'x') {
				switch (j) {
					...
					case 29:
						...
						break; //쿠폰율
					case 36:
						ex[e][j] = '<dt class="pic"><img src="http://mall.hyundailivart.co.kr/UserFiles/Image/01_promotion/common/error.jpg" alt=""/></dt>';
						break; //이미지url
					case 37:
						ex[e][j] = 'z1';
						break; //이미지 확대
				...
				switch (j) {
					...
					case 4:
						ex[e][j] = '<span>' + ex[e][j] + '</span>';
						break; //
					case 5:
						ex[e][j] = '<strong>' + ex[e][j] + '</strong>';
						break; //
					case 13:
						ex[e][j] = '	<dd class="txt">' + ex[e][j] + '</dd>';
						break; //설명문구
					case 14:
						ex[e][j] = addComma(Number(ex[e][j]));
						break; //정상가
					case 15:
						ex[e][j] = addComma(Number(ex[e][j]));
						break; //할인가
					...
					case 22: //출시여부
						if (ex[e][j] == 'soldout') {
							ex[e][j] = '<dd class="soldout"><img alt="" src="http://mall.hyundailivart.co.kr/UserFiles/Image/01_promotion/common/soldout.png"></dd>';
						} else if (ex[e][j] == 'upcoming') {
							ex[e][j] = '<dd class="soldout"><img alt="" src="http://mall.hyundailivart.co.kr/UserFiles/Image/01_promotion/common/upcoming.png"></dd>';
						}
						//ex[e][j]='['+ex[e][j]+']';
						break;
					case 24:
						if (ex[e][23] == 'ico00') {
							ex[e][24] = '' + '<table class="' + ex[e][23] + '">' + '	<tbody>' + '	  <tr>' + '		<td><strong>' + ex[e][24] + '</strong> <span>%</span></td>' + '	  </tr>' + '	</tbody>' + '</table>' + '';
						} else if (ex[e][23] == 'ico01') {
							ex[e][24] = '' + '		<table class="' + ex[e][23] + '">' + '				<tr>' + '					<td><span>무료<br/>배송</span></td>' + '				</tr>' + '		</table>' + '';
						} else if (ex[e][23] == 'ico02') {
							ex[e][24] = '' + '		<table class="' + ex[e][23] + '">' + '				<tr>' + '					<td><span>신상품</span></td>' + '				</tr>' + '		</table>' + '';
						} else {
							ex[e][24] = '';
						}
						break;
					case 27:
						if (ex[e][26] == 'ico00') {
							ex[e][27] = '' + '<table class="' + ex[e][26] + '">' + '	<tbody>' + '	  <tr>' + '		<td><strong>' + ex[e][24] + '</strong> <span>%</span></td>' + '	  </tr>' + '	</tbody>' + '</table>' + '';
						} else if (ex[e][26] == 'ico01') {
							ex[e][27] = '' + '		<table class="' + ex[e][26] + '">' + '				<tr>' + '					<td><span>무료<br/>배송</span></td>' + '				</tr>' + '		</table>' + '';
						} else if (ex[e][26] == 'ico02') {
							ex[e][2] = '' + '		<table class="' + ex[e][26] + '">' + '				<tr>' + '					<td><span>신상품</span></td>' + '				</tr>' + '		</table>' + '';
						}
						break;
					case 35:
						ex[e][j] = '<div class="lk"><a href="' + ex[e][j] + '"></a></div>';
						break; //url 열려있는 태그
					case 36:
						ex[e][j] = '<img src="' + ex[e][j] + '">';
						break; //이미지url
					case 37:
						ex[e][j] = ' ' + ex[e][j];
						break; //확대
			...
			if (j == 45) {
				...
				if (ex[e][2] == 'deal1') {
					...
					loop =
						...
						'		<p class="' +
						ex[e][2] +
						'"><img alt="" src="http://mall.hyundailivart.co.kr/UserFiles/Image/01_promotion/' +
						dayCode +
						'/' +
						ex[e][9] +
						'.' +
					...
					k = 2;
					for (l = 0; l < k; l++) {
						loop = loop + '		<li><img alt="" src="http://mall.hyundailivart.co.kr/UserFiles/Image/01_promotion/' + dayCode + '/' + oRange.Cells(e + l, 9) + '.' + oRange.Cells(e + l, 10) + '"></li>' + '';
						//alert(ex[e]);
						//e = Number(e + 1);
						//alert(ex[e]);
					}
					loop = '<!-- // box' + k + '영역 시작 부분 입니다.  -->' + '<div class="' + ex[e][3] + '">' + '	<ul class="box' + k + '">' + loop + '	</ul>' + '</div>' + '';
	...
	all.innerHTML = sum + '<div class="oWrap">' + osum + '</div>' + '';
}
