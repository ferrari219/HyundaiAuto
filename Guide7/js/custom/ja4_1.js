...
var k;
var brand = '';
var loc = 'http://100.10.10.200/04_ban/',
...
function excel(Location, i) {
	...
	Excel.Application.Workbooks.Open('\\\\100.10.10.200\\Lguidesystem\\04_ban\\' + sCode + '.xls');
	...
	function handleMaker(t, e, k) {
		...
		for (n = 0; n <= 16; n += 8) {
			...
			for (m = 1; m <= 5; m += 2) {
					...
					if (ex[e + i + n][k + m + 1] == '') {
						...
						switch (ex[e + i + n][k + m]) {
							...
							case '타이틀(소)':
								...
								break;
							case '타이틀(대)':
								if (Left(ex[e + i + n][k + m + 1], 4) == 'http') {
									temp += '<dl><dt><img src="' + ex[e + i + n][k + m + 1].trim() + '" alt="오브젝트" /></dt></dl>';
								} else {
									temp += '<dl><dt>' + ex[e + i + n][k + m + 1] + ' </dt></dl>';
								}
								break;
							case '설명':
				...
				Temp = Temp + '<div class="' + position + '" style="' + lOpt + tOpt + zidx + '">' + temp + '</div>';
				// '<br/>'+
				('');
				temp = '';
				lOpt = '';
		...
		Temp = '' + '<div class="sec ' + t + '">' + '	<h2>' + t + '배너</h2>' + '	<div class="frame" style="' + list[1] + '">' + Temp + '	</div>' + '</div>' + '';

		// Temp = ''+
		// '<div class="sec '+t+'">'+
		// '	<h3>'+t+'배너</h3>'+
		// '	<div class="frame" style="background:#b8af9e">'+
		// //url(http://100.10.10.200/04_ban/test/bg1.jpg) no-repeat center center;
		// '		<div class="CT" style="left:50%;top:0%;">'+
		// //			list.LT.Content4+
		// '			<dl>'+
		// '				<dt>딱 3일만 주는 혜택</dt>'+
		// '				<dd><span>설명글이 주저리주러리</span></dd>'+
		// '				<dd class="coupon"><img src="http://100.10.10.200/04_ban/test/coupon2.png" alt="쿠폰" /></dd>'+
		// '			</dl>'+
		// '			<p class="round btn">버튼</p>'+
		// '		</div>'+
		// '		<div class="RB" style="right:10%;bottom:0%;">'+
		// '			<p class="obj"><img src="http://100.10.10.200/04_ban/test/obj2.png" alt="오브젝트" /></p>'+
		// '		</div>'+
		// '	</div>'+
		// '</div>'+
